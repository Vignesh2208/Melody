...
class proxyIPCLayer(threading.Thread):
    ...
    def init_shared_ipc_buffers(self):
        for hostID in self.hostList:
            self.log.info("Opening shared buffer for : " + str(hostID))
            result = self.sharedBufferArray.open(bufName=str(hostID) + "buffer", isProxy=True)
            if result == BUF_NOT_INITIALIZED or result == FAILURE:
                self.log.info("Shared Buffer open failed for Proxy. Buffer not initialized")
            else:
