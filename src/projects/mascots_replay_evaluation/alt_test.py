...
def measure_dnp3_latencies(project_name, pcap_file_name):
    ...
    if p.data:
        print p.data
        if (p.data[5:]): # print stats for more than samples
...
def main():
    ...
    bg_flows = [
        ...
        EmulatedTrafficFlow(type=TRAFFIC_FLOW_ONE_SHOT,
                            ...
                            client_expect_file="ping -c" + str((run_time-1)*100) + " 10.0.0.2",
                            long_running=True),
        ...
        EmulatedTrafficFlow(type=TRAFFIC_FLOW_ONE_SHOT,
                            ...
                            server_process_start_cmd="",
                            #client_expect_file="ping -c" + str(run_time-1) + " 10.0.0.2",
                            client_expect_file="ping -c" + str((run_time-1)*100) + " " + network_configuration.mininet_obj.get("h3").IP(),
                            long_running=True),
